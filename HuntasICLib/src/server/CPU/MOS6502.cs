...
public class MOS6502 {
    ...
    // Treated as a different instruction
    const int JMPIND = 56;
    ...
    public bool invalidInstruction = false; // Gets set if the cpu executed an invalid instruction.
    ...
    public void UpdateLogic(){
        ...
        if(!soState && lastOFpinState){
            st |= 0b01000000;
        }
        lastOFpinState = soState;
        
        // Set phi1 and phi2 depending on if the clock turned off or on
        if(phi0State && !lastClkState){
            phi2 = true;
        } else if(!phi0State && lastClkState){
            phi1 = true;
        }
        lastClkState = phi0State;
        ...
        if(state == 0 && phi2){
            ...
            if(resetTriggerInt || (!irqState&&(st&0x04)==0 || !nmiState)&&!insideInterrupt){
                if(resetTriggerInt){
                    interruptType = 0;
                } else if(!nmiState){
                    interruptType = 1;
